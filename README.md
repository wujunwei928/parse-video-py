...
YouTube 解析能力参考了以下 GitHub 仓库的通用做法（通过提取器抽取元信息并选择最佳格式直链）：

- https://github.com/yt-dlp/yt-dlp
- https://github.com/ytdl-org/youtube-dl
- https://github.com/pytube/pytube

当前项目实现未直接依赖 `yt-dlp` 命令行，而是参考其开源解析思路：优先从页面提取播放器 JSON，必要时回退 Innertube 多客户端重试（用于规避部分 `LOGIN_REQUIRED` 场景），再选择最优视频格式，支持 `youtube.com` / `youtu.be` / `shorts` 链接解析并返回可下载视频直链。
